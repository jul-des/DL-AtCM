...
class ECGRegressionModel(nn.Module):
    ...
    """
    def __init__(self, pretrained_model, feature_dim=768, num_outputs=2, dropout_rate=0.2):
        super(ECGRegressionModel, self).__init__()
...
def validate_epoch(model, dataloader, criterion, device):
    ...
    mse_per_dim = [mean_squared_error(targets[:, i], predictions[:, i]) for i in range(targets.shape[1])]
    mae_per_dim = [mean_absolute_error(targets[:, i], predictions[:, i]) for i in range(targets.shape[1])]
    r2_per_dim = [r2_score(targets[:, i], predictions[:, i]) for i in range(targets.shape[1])]
    r_per_dim = [np.corrcoef(targets[:, i], predictions[:, i])[0,1] for i in range(targets.shape[1])]

    return avg_loss, mse_per_dim, mae_per_dim, r2_per_dim, r_per_dim, predictions, targets
...
def finetune_model(pretrained_model, train_loader, val_loader, device, criterion,
    ...
    model = ECGRegressionModel(pretrained_model, num_outputs=num_out, dropout_rate=dropout_rate)
    ...
    for epoch in range(epoch_probe):
        ...
        val_loss, val_mse, val_mae, val_r2, val_r, _, _ = validate_epoch(model, val_loader, criterion, device)
        ...
        print(f"Val MSE per dim: {[f'{mse:.4f}' for mse in val_mse]}", flush=True)
    ...
    for epoch in range(num_epochs - epoch_probe):
        train_loss = train_epoch(model, train_loader, criterion, optimizer, device)
        val_loss, val_mse, val_mae, val_r2, val_r, predictions, targets = validate_epoch(model, val_loader, criterion, device)
        ...
        print(f"Val MSE per dim: {[f'{mse:.4f}' for mse in val_mse]}", flush=True)
        print(f"Val MAE per dim: {[f'{mae:.4f}' for mae in val_mae]}", flush=True)
        print(f"Val R2 per dim: {[f'{r2:.4f}' for r2 in val_r2]}", flush=True)
        print(f"Val Corr. per dim: {[f'{r:.4f}' for r in val_r]}", flush=True)
        print("Learning Rate: " + str(current_lr))

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), "/home/dnanexus/best_ecg_regression_model.pt")
