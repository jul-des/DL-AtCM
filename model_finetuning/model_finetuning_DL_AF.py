...
path_to_tabular_data = "path/to/tabular" #Path to tabluar data. Should be a csv file containing a column "AF" which indicates whether patients have previous diagnosis of AF or not and 
#and a column "patient_ids", containing patient ids and a column  "ecg_files" containing the names of the preprocessed and segmented ecg files to be included in train, validation and testing.

path_to_ECGs = "path/to/ecgs" #path to a folder containing all the preprocessed and segmented ECGs 

path_to_outputs = "path/to/outputs" #path to folder where outputs should be saved

path_to_foundation_model = "path/to/fm" #path were the ECG-FM mimic_iv_ecg_physionet_pretrained.pt model is saved. 
...
#Load Foundation model from McKeen et al.
...
finetuned_model, losses = finetune_model(pretrained_model, train_loader, val_loader, device, criterion=nn.BCEWithLogitsLoss(), num_out=1, num_epochs=config["training"]["num_epochs_total"], epoch_probe = config["training"]["num_epochs_regression_head"], 
